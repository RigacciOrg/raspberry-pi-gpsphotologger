...
class IfdGPS(IfdData):
    ...
    tags = {
        ...
        0xC: ("Speed: [K]ilometers, [M]iles or k[N]ots", "GPSSpeedRef", ASCII, 2),
        ...
        ## 1B MISSING
        ## 1C MISSING
        0x1d: ("Date relative to UTC", "GPSDateStamp", ASCII, 11),
        ## 1E MISSING
        }
...
class JpegFile:
    ...
    def set_altitude(self, elevation):
        ...
        if elevation >= 0:
            ...
            ref = '\x01'
            val = -elevation
        gps.GPSAltitude = [Rational(val * 10, 10)]
    ...
    def set_timestamp(self, hour, minute, second):
        """Set the time as UTC"""

        gps = self.exif.primary.GPS
        gps.GPSTimeStamp = [Rational(hour, 1),
                            Rational(minute, 1),
                            Rational(second, 1)]
